...
class Trainer(nn.Module):
    def __init__(self, vocab_size, embedding_size, ini_weight, hidden_dim, bidirection = False, option_number=4):
        ...
        self.p2q_attention = attention_layer.BilinearAttentionP2Q(self.weight_mtx_dim )
        self.opt2p_attention = attention_layer.BilinearAttentionO2P(self.weight_mtx_dim )
    ...
    def forward(self, passage, msk_p, lst_p, qst,msk_qst,lst_qst,opt,msk_opt,lst_opt):
        ...
        p2q_align = self.p2q_attention(encoded_passage,qst_ht)  # batch x sentence_len x 1

        p2q_align = torch.unsqueeze(p2q_align,2)
        p2q_align = p2q_align * msk_p

        p_expectation = torch.sum(p2q_align*encoded_passage,dim=1) # batch x emb_size
        #print p_expectation

        o2p_align = self.opt2p_attention(opt_ht,p_expectation)
        return o2p_align
