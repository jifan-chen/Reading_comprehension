...
class RNN_encoder():
    ...
    def __init__(self, hidden_size, vscope, bidirectional, input_keep_prob, output_keep_prob,reuse=False):
        ...
        if self.bidirectional:
            with tf.variable_scope(self.vscope):
                with tf.variable_scope('forward_rnn'):
                    ...
                    lstm_fw_cell = rnn.DropoutWrapper(lstm_fw_cell,input_keep_prob=self.input_keep_prob,output_keep_prob=self.output_keep_prob)
                # Backward direction cell
                with tf.variable_scope('backward_rnn'):
                    lstm_bw_cell = rnn.GRUCell(self.hidden_size,reuse=self.reuse)
                    lstm_bw_cell = rnn.DropoutWrapper(lstm_bw_cell,input_keep_prob=self.input_keep_prob,output_keep_prob=self.output_keep_prob)
            ...
            with tf.variable_scope(self.vscope):
                ...
                lstm_cell = rnn.DropoutWrapper(lstm_cell,input_keep_prob=self.input_keep_prob,output_keep_prob=self.output_keep_prob)
